function id_out = id_unicycle(results)
    ...
    % --- CORREZIONE 1: Inversione assi motion capture (y e z opposti) ---
    q_motion_capture(:, 2) = -q_motion_capture(:, 2); 
    q_motion_capture(:, 3) = -q_motion_capture(:, 3);

    % Preparazione array di identificazione
    q4id = q_motion_capture;  
    q4id(:,3) = unwrap(q_motion_capture(:,3));  % Corregge i salti di ±2π

    % --- CORREZIONE 2: Scambio ruota destra/sinistra ---
    omega_wheels = ws_meas(:, [2, 1]);  
    ...
    figure('Name', 'Confronto Stime Cinematiche', 'Color', 'w')
