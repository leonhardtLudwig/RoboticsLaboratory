function plot_unicycle_trajectory_calibrated(trajectory_ideal, trajectory_actual, trajectory_calibrated)
    ...
    %   trajectory_calibrated - 3xK matrix (x, y, theta) of the calibrated trajectory

    % Check dimensions
    if size(trajectory_ideal, 1) ~= 3 || size(trajectory_actual, 1) ~= 3 || size(trajectory_calibrated, 1) ~= 3
        error('Error: All input matrices must have 3 rows (x, y, theta).');
    end
    ...
    x_cal = trajectory_calibrated(1, :);
    y_cal = trajectory_calibrated(2, :);
    theta_cal = trajectory_calibrated(3, :);
    ...
    plot(x_cal, y_cal, 'g-.', 'LineWidth', 1.5, 'DisplayName', 'Calibrated');
    
    % --- DYNAMIC SIZING FOR UNICYCLE SYMBOLS ---
    % Calculate the max range of the plot to scale the triangles appropriately (including calibrated)
    max_x = max([x_ideal, x_actual, x_cal]); min_x = min([x_ideal, x_actual, x_cal]);
    max_y = max([y_ideal, y_actual, y_cal]); min_y = min([y_ideal, y_actual, y_cal]);
    ...
    % Function to plot symbols along a trajectory
    ...
    % Plot symbols for Calibrated (Green)
    plot_symbols(x_cal, y_cal, theta_cal, 'g');
