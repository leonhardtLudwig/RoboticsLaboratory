...
% test localization strategies based on integration and based on Kalman filter 
% with Discrete-Time measurements and with displacemnt in the parameters

% consider Ts = 0.1 / 0.04 / 0.001
% with / without initialization error
% p_loss = 0.99 / 0.9
...
%% 1. SYSTEM SETUP & TRAJECTORY GENERATION
% Nominal parameters
r = 0.03;
d = 0.165;
...
% Get q trajectory with differential flatness
...
%% 2. PARAMETERS DISPLACEMENT & EKF INIT (Task 1.7 & 1.8)
T_s = 0.04;

% Actual parameters (Displacement)
r_actual = 0.031;
d_actual = 0.164;
...
D = diag([0.001, 0.001, 0.0175/6, 0.0175/6, 0.0175/6, 0.0175/6*T_s, 0.0175/6*T_s].^2);
R_2 = diag([ENCODER_QUANTIZATION/6, ENCODER_QUANTIZATION/6].^2);
R_4 = diag([noise_std, noise_std, ENCODER_QUANTIZATION/6, ENCODER_QUANTIZATION/6].^2); % Corrected to 0.01
...
%% 3. RUN SIMULATION AND SAVE DATA
simulink_model_name = 'Sim_localization_gps_1_8'; 
ploss_values = [0.9, 0.99];

% Preallocate structure
opt1_data(length(ploss_values)) = struct();

fprintf('Running simulations to generate data...\n');
for i = 1:length(ploss_values)
    p_loss = ploss_values(i);
    fprintf('Simulating with p_loss = %.2f\n', p_loss);
    
    % Run simulation
    out = sim(simulink_model_name);
    
    % Store data
    opt1_data(i).p_loss = p_loss;
    opt1_data(i).q_actual = out.q.signals.values;
    opt1_data(i).z_estimate = out.z_EKF.signals.values;
end

% Save data to a .mat file
save('opt1_results.mat', 'opt1_data');
fprintf('Data successfully saved to opt1_results.mat.\n\n');

%% 4. LOAD DATA AND PLOT 
fprintf('Loading data and plotting...\n');
load('opt1_results.mat');

for i = 1:length(opt1_data)
    q_act = opt1_data(i).q_actual;
    z_est = opt1_data(i).z_estimate;
    p_loss_val = opt1_data(i).p_loss;
    
    % Use the existing plotting function (ignoring exact odometry for this specific plot)
    % Se la tua funzione richiede 3 input, passiamo q_act due volte o creiamo un plot custom.
    % Per pulizia, creiamo una figura dedicata solo a Verità vs EKF.
    
    figure('Name', sprintf('OPT 1: p_{loss} = %.2f', p_loss_val));
    hold on; grid on;
    
    % Actual Trajectory
    plot(q_act(1,:), q_act(2,:), 'b-', 'LineWidth', 1.5, 'DisplayName', 'Actual Trajectory');
    
    % EKF Estimated Trajectory
    plot(z_est(1,:), z_est(2,:), 'r--', 'LineWidth', 1.5, 'DisplayName', 'EKF Estimate');
    
    title(sprintf('EKF vs Actual (GPS p_{loss} = %.2f)', p_loss_val));
    xlabel('x [m]');
    ylabel('y [m]');
...
disp('Optional 1 completely executed.');