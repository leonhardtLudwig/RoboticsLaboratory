function compare_multiple_tracking_experiments(exps, exp_labels, start_fig)
    % COMPARE_MULTIPLE_TRACKING_EXPERIMENTS Confronta N esperimenti su 4 plot separati.
    % Adattato per il tracking: mostra q_des e usa i simboli a triangolo.
    ...
    % Rimosso il testo "INITIAL" per mantenere la pulizia
    ...
    % --- 1.2 Parametri Geometrici Robot (per triangoli) ---
    ...
    % Plotta la traiettoria desiderata (basta prenderla dal primo esperimento)
    if isfield(exps{1}, 'q_des')
        plot(exps{1}.q_des(1, :), exps{1}.q_des(2, :), 'k--', 'LineWidth', 1.5, 'DisplayName', 'Desired Trajectory');
    end
    ...
    title('EKF Trajectories Comparison (Tracking)', 'FontSize', 14);
    ...
    % =========================================================================
    % PLOT 3: TRACKING ERROR (EKF vs DESIRED)
    % =========================================================================
    fig3 = figure(start_fig + 2); clf(fig3);
    set(fig3, 'Name', 'Tracking Errors Comparison', 'Color', 'w');
    sgtitle('Tracking Errors (EKF vs Desired)', 'FontWeight', 'bold');
    ...
    % Subplot per Velocità Lineare (v)
    subplot(2, 1, 1); hold on; grid on;
    for i = 1:num_exps
        N = size(exps{i}.u, 1);
        t = (0:N-1) * Ts;
    ...
    % Subplot per Velocità Angolare (omega)
    subplot(2, 1, 2); hold on; grid on;
    for i = 1:num_exps
        N = size(exps{i}.u, 1);
        t = (0:N-1) * Ts;
